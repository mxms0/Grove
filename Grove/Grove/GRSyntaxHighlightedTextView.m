...
- (NSRange)attributeRangeBeginningWithString:(NSString *)string endingWithCharacterSet:(NSCharacterSet *)endSet queryRange:(NSRange)inRange {
	...
	NSRange endRange = [self.text rangeOfCharacterFromSet:endSet options:0 range:NSMakeRange(beginningRange.location + beginningRange.length, [self.text length] - (beginningRange.location + beginningRange.length))];
